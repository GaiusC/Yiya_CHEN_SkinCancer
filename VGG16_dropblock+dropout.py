...
def hair_remove(image):
    ...
    final_image = cv2.inpaint(image, threshold, 5,
                              cv2.INPAINT_TELEA)  # Fast Marching Method  5 is value of  inpaintRadius
    return final_image
...
mcp = tf.keras.callbacks.ModelCheckpoint(monitor='val_accuracy', patience=6, verbose=1,filepath='./vgg16_model.h5',save_best_only=True,mode='max')
...
class_weights = np.around(
    compute_class_weight(class_weight='balanced', classes=np.unique(y_id), y=skin_df_balanced['cell_type_idx']), 2)
...
batch_size = 32
vgg16_base = tf.keras.applications.VGG16(include_top=False, weights='imagenet', input_tensor=None,
                                         input_shape=(224, 224, 3))
...
# mcp=tf.keras.callsback.ModelCheckpoint(monitor='val_accuracy',patience=6,verbose=1,filepath='./VGG16_model.h5',
#                                       save_best_only=True,mode='max')

VGG16_model_dropout.compile(optimizer=optimizer, loss="categorical_crossentropy", metrics=["accuracy"])
history_VGG16 = VGG16_model_dropout.fit(x_train, y_train, batch_size=batch_size, epochs=30,
